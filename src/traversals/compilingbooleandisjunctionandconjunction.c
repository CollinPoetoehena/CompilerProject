...
node_st *CBDCbinop(node_st *node)
{
    ...
    if (BINOP_OP(node) == BO_or) {        
        // Use the type signature for the BinOp node obtained in TypeChecking
        // pred == true ? then not evaluate then expr (true expr) : otherwise evaluate else expression to find true
        ...
        // Use the type signature for the BinOp node obtained in TypeChecking
        // pred == true ? then evaluate then expr to also find true : otherwise not evaluate else expression
